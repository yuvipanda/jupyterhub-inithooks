...
def test_hook_exec():
    ...
    with TemporaryDirectory() as d:
        ...
        cmd = [
            ...
            "/bin/bash",
            "-c",
            "id -u > ${TEST_HARNESS_DIR}/uid && id -g > ${TEST_HARNESS_DIR}/gid",
        ]
        env = os.environ.copy()
        ...
        with open(hi_path) as hi, open(hello_path) as hello, open(
            Path(d) / "hello"
        ) as hello, open(Path(d) / "uid") as uid, open(Path(d) / "gid") as gid:
            assert hi.read() == "hi"
